...
/**
 * @file plugins/themes/saha/SahaThemePlugin.inc.php
 *
...
class SahaThemePlugin extends ThemePlugin
{
	/**
	...
	public function init()
	{
		...
		$UCLLogo = $this->getPluginPath() . "/templates/images/uc-logo.png";

		// Register theme options
		$this->addOption('latestArticlesNumber', 'text', array(
			'label' => 'plugins.saha.latest.number',
			'description' => 'plugins.saha.latest.description',
		));

		$this->addOption('displayIssuesSlider', 'text', array(
			'label' => 'plugins.saha.if-display.issue-slider',
			'description' => 'plugins.saha.if-display.issue-slider.description',
		));

		$this->addOption('journalSummary', 'radio', array(
			'label' => 'plugins.saha.journal.summary.display',
			'options' => array(
				'true' => 'plugins.saha.journal.summary.display.true',
				'false' => 'plugins.saha.journal.summary.display.false',
			)
		));
	...
	{
		return __('plugins.themes.saha.name');
	}
	...
	{
		return __('plugins.themes.saha.description');
	}
	...
	public function latestIssuesSlider($hookName, $args) {
		...
		if (is_null($latestIssuesInput)) {
			$latestIssuesInput = SAHA_LATEST_ISSUES_DEFAULT;
